...
export function audioSettingsLayout(w: number, h: number, insets: Insets): AudioSettingsLayout {
  ...
  const panelH = Math.min(200, availableH - 24);
  const x = insets.left + (availableW - panelW) / 2;
  // Position panel at bottom, above the fold
  const y = insets.top + availableH - panelH - 8;
  const labelW = 56;
  ...
  return {
    ...
    sfx: { x: trackX, y: y + 56, w: trackW, h: 32 },
